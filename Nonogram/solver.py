...
def deduce(hr, vr, solution):
    ...
    def show_gram(m):
        ...
        for x in m:
            ...
            finalSolution.append((" ".join("x#.?"[i] for i in x)).split(" "))
    ...
    def try_all(solution, n = 0):
        if n >= h:
            for j in range(w):
                if [x[j] for x in out] not in cols[j]:
                    return 0
            solution.append(show_gram(out))
            return 1
        sol = 0
        for x in rows[n]:
            out[n] = x
            sol += try_all(solution, n + 1)
        return sol
